/*!
 *	@brief Example shows basic application of configuring and reading the data ready interrupt.
 */
...
int main(void)
{
	...
	/* Set the desired interrupt configuration in the sensor */
	rslt = bmg250_set_int_config(&int_conf, &gyro);

	/* Reading the interrupt */
	while (1) {
		rslt = bmg250_get_int_status(&intr_status, &gyro);

		/* checking for data ready interrupt */
		if (intr_status == DRDY_INT_ASSERTED) {
